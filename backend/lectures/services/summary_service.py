...
"""
    ...
    if subject_code == "auto":
        return f"""너는 강의 내용을 분석하여 시험 대비 요약을 만드는 전문 강사 AI이다.
아래는 과목 유형을 사용자가 직접 지정하지 않은 강의의 일부 구간이다.
강의 내용을 보고 역사/인문학, 프로그래밍/IT, 수학/과학, 일반 강의 중 가장 가까운 성격을 스스로 판단한 뒤 시험 대비에 적합하게 요약하라.

{base_instructions}

[AI 자동 판단 요구사항]
- 이 구간의 과목 성격을 먼저 추정하되, 출력은 시험 공부에 바로 쓸 수 있는 요약 중심으로 작성할 것.
- 암기해야 할 용어, 개념 관계, 실습 흐름, 공식, 사건 흐름 등 강의 내용에 맞는 학습 포인트를 자동으로 선택할 것.
- 단순 줄거리 요약보다 시험에 나올 수 있는 핵심 개념, 비교 포인트, 자주 헷갈릴 부분을 중심으로 정리할 것.
- 과목이 혼합되어 있으면 가장 많이 다뤄진 내용 기준으로 요약하고, 필요한 경우 보조 관점도 함께 적을 것.
...
"""
        ...
        if subject_code == "auto":
            subject_prompt = f"""당신은 강의 내용을 분석하여 시험 대비 학습 노트를 만드는 전문 강사 AI입니다.
다음은 사용자가 과목 유형을 직접 지정하지 않은 강의의 구간별 분석 내용입니다.
먼저 강의의 성격을 스스로 판단하고, 시험 대비에 가장 적합한 구조로 최종 요약 노트를 작성하세요.

{multimodal_instruction}

[AI 자동 판단 요구사항]
- 강의 내용을 바탕으로 역사/인문학, 프로그래밍/IT, 수학/과학, 일반 강의 중 가장 가까운 과목 성격을 판단할 것.
- 판단한 과목 성격에 따라 사건 흐름, 코드 흐름, 개념/공식, 핵심 주제 정리 중 적절한 요약 방식을 자동으로 선택할 것.
- 시험 준비에 활용할 수 있도록 핵심 개념, 출제 가능 포인트, 헷갈리기 쉬운 내용, 암기 포인트를 분명히 정리할 것.
- 단순한 강의 요약보다 학습자가 복습 자료로 바로 사용할 수 있는 노트 형태로 작성할 것.
- 과목 성격이 혼합되어 있으면 가장 중요한 흐름을 기준으로 정리하고, 보조적으로 필요한 관점을 함께 설명할 것.

[출력 양식]
1. AI가 판단한 강의 유형 및 전체 요약
- 강의의 과목 성격과 핵심 주제를 먼저 밝히고, 전체 흐름을 3~5개 문단으로 정리할 것.

2. 시험 대비 상세 정리
- [도입] - [핵심 개념] - [세부 설명] - [정리] 흐름으로 정리할 것.
- 제공된 이미지가 설명에 직접 도움이 되는 경우에만 [IMG: 분:초] 마커를 삽입할 것.

3. 꼭 알아야 할 필수 개념
- 시험 대비에 중요한 키워드를 7~10개 선정할 것.
- 키워드 이름 (분:초)
  - 구체적 의미 및 핵심 내용
  - 시험에서 헷갈리기 쉬운 부분 또는 연결 개념

4. 핵심 출제 포인트
- 출제 가능성이 높은 내용, 비교 포인트, 암기 포인트, 실수하기 쉬운 내용을 불렛포인트로 정리할 것.
"""
